...
const contadorProductos = document.getElementById("contadorProductos"); //Contador sobre los  productos, (no son de unidades)
...
//Se presenta la numeración de la primera columna de la tabla
let cont = 0;
let costoTotal=0;
let totalEnProductos = 0;

let datos = new Array(); //Aqui se almacena todos los elementos de la tabla
...
function validarCantidad(){
    ...
    if(Number(txtNumber.value)<=0){ //Primero se convierte a Number, y despues nos aseguramos que la cantidad sea mayor a 0
        return false;
    }//menor igual a 0
    return true;
}//validarCantidad
...
btnAgregar.addEventListener("click", function(event){
    ...
    //Bandera, al salir un resultado true nos permite agregar los datos a la tabla
    let isValid=true;

    //Limpiando formatos de alerta e input
    txtName.style.border=""; //no cuenta con estilo para nombre
    txtNumber.style.border=""; //no cuenta con estilo para cantidad
    alertValidacionesTexto.innerHTML="" //no hay mensaje
    alertValidaciones.style.display="none";  //oculto

    txtName.value = txtName.value.trim(); //Aqui se quitan espacios vacios al inicio y al final
    ...
    if(isValid){
        ...
        let elemento =  { //Creando el -objeto- con la notación JSON (clave, valor) a partir de los datos de la tabla
                            "cont" : cont,
                            "nombre" : txtName,
                            "cantidad" : txtNumber,
                            "precio" : precio 
        ...
        //Aqui para crear el objeto despues de la asignacion de sus valores
        let resumen =   { //Creando objeto para el resumen de compra
            "cont": cont,
            "totalEnProductos": totalEnProductos,
            "costoTotal": costoTotal
        }
        //Almacenando todo el resumen al localstorage
        localStorage.setItem("resumen", JSON.stringify(resumen));

        //Aqui se limpia los campos después de agregarlos a la tabla
        txtName.value="";
        txtNumber.value="";
        //se muestra como regresa el puntero a el primer campo para seguir escribiendo
        txtName.focus(); 
    }
...
// La funcion para limpiar todo (resumen, datos, campo y agregar alerta)
